...
class GaussianMaskInpainting(nn.Module):
    ...
    def _single_mask(self, beta: torch.Tensor, cx: torch.Tensor, cy: torch.Tensor) -> torch.Tensor:
        ...
        return 1.0 - g  # peak at center is 0

    def cumulative_mask(self, t: torch.Tensor, generator: torch.Generator | None = None) -> torch.Tensor:
        """Build the cumulative mask M_t = ∏_{i=1..t} z_{β_i} per image.

        t: (B,) long tensor of severities in [0, T].
        Returns (B, 1, H, W) mask in [0, 1]. Pixels where M_t ≈ 0 are masked.
        """
        device = t.device
        B = t.shape[0]
        ...
        if self.randomize_center:
            cx = torch.randint(0, self.image_size, (B,), device=device, generator=generator).float()
            cy = torch.randint(0, self.image_size, (B,), device=device, generator=generator).float()
        else:
    ...
    def forward(self, x0: torch.Tensor, t: torch.Tensor, generator: torch.Generator | None = None) -> torch.Tensor:
        mask = self.cumulative_mask(t, generator=generator)
        return x0 * mask
